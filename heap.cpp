...
#define STATIC_ARRAY_SIZE 217

const char* static_arr[STATIC_ARRAY_SIZE] = {
  // "Б",
  // "Д",
  // "А",
  // "В",
  ...
  // "Г",
  // "Е",
  "Петро Юрій Володимирович 1988.03.05",
...
void heapify(char* arr[], int start, int end) {
  int largest = start;
  int left = 2 * start + 1;
  int right = 2 * start + 2;

  if (left < end && strcmp(arr[left], arr[largest]) > 0) {
    largest = left;
  }

  if (right < end && strcmp(arr[right], arr[largest]) > 0) {
    largest = right;
  }
...
void heapSort(char* arr[], int start, int end) {
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < count; i++) {
    heap[i] = const_cast<char*>(static_arr[i % STATIC_ARRAY_SIZE]);
  }
